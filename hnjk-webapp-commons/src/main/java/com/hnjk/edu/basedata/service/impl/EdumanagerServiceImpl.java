...
//		}
	...
	public String getTeacherCode(String teacherCodeRule, OrgUnit unit) {
		...
		if ("Y".equals(isUseUnitName)) {
			teacherCode.append(ExStringUtils.getPinYinHeadChar(unit.getUnitShortNameForCustom()));
		} else {
		...
		try {
			...
			builder.append("  join hnjk_sys_roleusers ru on ru.USERID=u.RESOURCEID join HNJK_SYS_ROLES r on r.RESOURCEID=ru.ROLEID and r.ROLECODE like 'ROLE_TEACHER%'");
