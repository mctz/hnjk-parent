...
<head>
<title>学生网上报名表单</title>
<link type="text/css" rel="stylesheet"
	href="${baseUrl}/style/default/netedu3-default.css" />
<gh:loadCom components="tab,validator,blockUI,cnArea,datePicker" />
</head>
<body>
	<script type="text/javascript">
      		
    var brSchoolId = '${brSchoolId}';//默认教学站
	...
	jQuery(document).ready(function(){	 		
        ...
        jQuery('#next').click(function(){//下一步事件
          ...
          jQuery('#prev').click(function(){//上一步事件
                tab.triggleTab(0) ;
                 return false;
           });
       ...
       jQuery('#save').click(function(){                  	
			...
			var certNum = $("#enrolleeinfo_add_table input[id=certNum]").val();
			//校验身份证唯一性及合法性
			if(""==certNum){
				  alert("请填写一个证件号码！")
					  return false;
			 		  ...
			 		  $.get(checkExistsURL,{certNum:certNum},function(data){
			 			  if("true"==data){
			 				  $("#enrolleeinfo_add_table input[id=isExstisCertNum]").attr("value","true");
			 			  }else{
			 				  $("#enrolleeinfo_add_table input[id=isExstisCertNum]").attr("value","false");
			 			  }
			 		  })
			  		  var isExists = $("#enrolleeinfo_add_table input[id=isExstisCertNum]").val();
			  		  if("true"==isExists){
			  			  alert("该证件号已存在！");
			  			  return false;
			 		  }
          ...
          /*
		    ...
		    */  
			...
			// 	 aspnet:true,
			 ...
			 jQuery("#branchSchoolid").change(function(){
			...
			jQuery("#recruitPlanId").change(function(){
				...
				if(recruitPlanId==null||recruitPlanId==""){
					flag = false;
					jQuery("#branchSchoolid").children().remove();
					alert("请选择招生批次!");
				}
				if(flag != false){
					jQuery.post(url,{recruitPlanId:recruitPlanId},function(myJSON){
							jQuery("#branchSchoolid").children().remove();
							jQuery("#recruitMajorid").children().remove();
							//jQuery("#branchSchoolid").show();
						  	 var selectObj="<option value=''>请选择教学站</option>";
						 	 for (var i = 0; i < myJSON.length; i++) {
						 	 	selectObj += '<option value="' + myJSON[i].key + '">' + myJSON[i].value + '</option>';    
								}
						 	 jQuery("#branchSchoolid").html(selectObj);
					},"json");
				}
			});
		...
		function getExamSubjectList(recruitPlanId,recruitMajorId){
			...
			jQuery.post(url,{recruitMajorId:recruitMajorId,recruitPlanId:recruitPlanId},function(myJSON){
					if(myJSON.length>0){
						for(var i=0;i<myJSON.length;i++){
							examSubJect += "<tr><td>"+(i+1)+"</td><td>"+myJSON[i].courseName
										+"</td><td>"+myJSON[i].startTimeStr+"-"+myJSON[i].endTimeStr+"</td></tr>"
						}	
					}else{
		...
		if(15==val.length){ //15位身份证号码
		  ...
		  if(18==val.length){ //18位身份证号码
		    ...
		    if(val.charAt(17)!=IDCard(val))  {
		     ...
		     $("#certNum").css({"backgroundColor":"#ffc8c8"});
		     _gender.val("");//清除
		     _bornDay.val("");
		    }else{
		    	$("#certNum").css({"backgroundColor":"#fff"});	
		    	_bornDay.val(birthdayValue);
		    }
				...
				<div class="ui-tab-container">
					...
					<div class="ui-tab-bd">
							...
							<table class="form" id="fragment-1">
								<tr>
									...
									<td width="40%"><select id="branchSchoolid"
										name="branchSchool" mes="教学站" validate="Require">
											<option value="">请选择教学站</option>
									</select> <font color='red'>* 选择您的学习地点</font></td>

								</tr>
								<tr>
									<td>招生专业:</td>
									<td colspan="3"><select id="recruitMajorid"
										name="recruitMajor" onchange="grnerateExamCode();"
										validate="Require" mes="招生专业">
											<option value="">请选择专业</option>
									</select> <font color='red'>*</font></td>
									<%-- <td>准考证号:</td>
										<td>
											<input type="text" id="examCertificateNo" name="examCertificateNo" size="34"  value="${enrolleeinfo.examCertificateNo }" readonly="true" validate="Require" mes="准考证号"/>
										</td>--%>
								</tr>
								<tr>
									<td>申请免试:</td>
									...
									<td><gh:select name="educationalLevel"
											...
											mes="入学前国民教育最高学历层次" /> <font color='red'>*</font></td>
								</tr>
								...
								<tr id="graduateInfoTr" style="display: none;">
									...
									<td><input type="text" id="graduateId" name="graduateId"
										size="34" value="${enrolleeinfo.graduateId }" mes="入学前学历证书编号" /><font
										color='red'>*</font></td>
									<td>毕业专业:</td>
									<td><input type="text" name="graduateMajor" size="34"
										value="${enrolleeinfo.graduateMajor }" mes="毕业专业" /><font
										color='red'>*</font></td>
								</tr>
								<tr>
									<td>入学前最高学历毕业日期:</td>
									<td><input type="text" name="graduateDate" size="34"
										class="Wdate" validate="Require,Date2"
									...
									<td><gh:select id="fromMedia" name="fromMedia"
											...
											value="${enrolleeinfo.fromMedia}" mes="信息地媒体来源" /> <input
						...
						<div class="ui-tab-content" style="display: none">
							...
							<table id="enrolleeinfo_add_table" class="form"
								style="width: 100%">
								<tr>
									<td width="10%">姓名:</td>
								...
								<tr>
									<td>出生日期:</td>
									<td><input type="text" id="_bornDay" name="bornDay"
										size="34" readonly="readonly" value="${student.bornDay }"
										class="Wdate" validate="Require,Date2"
										onfocus="WdatePicker({isShowWeek:true})" mes="出生日期" /> <font
										color='red'>*</font></td>
									<td>性别:</td>
									<td><gh:select name="gender" id="_gender"
											dictionaryCode="CodeSex" value="${student.gender}"
											validate="Require" mes="性别" /> <font color='red'>*</font></td>
								</tr>
								<tr>
									<td>籍贯:</td>
									<td>
								...
								<tr>
									...
									<td><gh:select id="workStatus" name="workStatus"
											...
											mes="职业状况" /><font color='red'>*</font></td>
									<td>从业行业:</td>
								...
								<tr>
									...
									<td><input type="text" name="contactAddress" size="34"
										...
										validate="Require,LimitLen(max:200)" mes="本人地址" /> <!--  <font color='red'>* 用于寄送通知书,请填写准确</font>-->
								...
								<!--  
									<tr>						
										...
										<td>
										 ...
										 <input type="file" id="uploadPhoto" name="uploadPhoto"  size="40"/>
										</td>
										<td>上传身份证复印件:</td>
										<td>
										 <div id="filelist2"></div>						
										 <input type="file" id="idcardPhoto" name="idcardPhoto"   size="40"/>
										</td>						
									</tr>
									-->
								<tr>
									<!-- <td>上传毕业证复印件:</td>
										<td>
										 ...
										 <input type="file" id="graduationPhoto" name="graduationPhoto"  size="40"/>
										</td>
										 -->
									<td>备注说明:</td>
									<td colspan="3"><textarea name="memo" cols="30" rows="2"
								...
								<!-- 
									<tr>
										<td>图片预览：</td>						
										<td colspan="3">
										<div style="overflow: hidden;height:120px; border: 1px solid ccc;">
				       	 					<div style="width:100px;float:left;margin-left:4px"> <img id="uploadimage1" src="" /></div>
				       	 					<div style="width:100px;float:left;margin-left:4px"> <img id="uploadimage2" src="" /></div>
				       	 					<div style="width:100px;float:left;margin-left:4px"> <img id="uploadimage3" src="" /></div>			       			
							   			 </div></td>
									</tr>
									 -->
							</table>
