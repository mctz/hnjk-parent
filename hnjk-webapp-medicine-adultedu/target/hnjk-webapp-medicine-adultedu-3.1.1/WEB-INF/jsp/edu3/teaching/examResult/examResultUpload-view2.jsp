...
<body>
						...
						<div style="margin-left: 10px;">
							<font color="red">
								...
								<p style="font-weight: bolder">
									成绩异常：1.作弊、2.缺考、3.无卷、4.其它，请严格按照规范填写在卷面成绩栏里</p>
							</font>
						</div>
					</td>
					<td rowspan="2" width="40%">
						<div id="revocationImprotDIV"
							style="display: none; float: right; border: 2px dotted red; width: 200px; height: 80px; margin-right: 30px;">
							<span id="revocationImprotTypeSPN"
								style="margin-top: 10px; margin-left: 20px; float: left;">
								<input name="revocationType" value="0" checked="checked"
								type="radio" onclick="chooseRevocationType(0)" />当前批次 &nbsp; <input
								name="revocationType" value="1" type="radio"
								onclick="chooseRevocationType(1)" />按课程清空
							</span> <span id="revocationImprotCourseSPN"
								style="display: none; margin-left: 30px; margin-top: 10px; float: left;">
								<select id="examSubCourseSelect" name="examSubCourseSelect"
								onchange="revocationImprotByCourseProcess();">
							...
							if(success==false){
								...
								if(null!=saveList){
									for(var i=0;i<saveList.length;i++){
										$("#examResultsVoBody  input[name='importExamresoultsResourceid'][value='"+saveList[i]+"']").parent().parent().parent().html("");
									}
								}
								if(null!=errorList){
									alertMsg.warn(msg);
	...
	//保存并创建成绩记录
	function processImprotWithOutRecord(){
		var importExamresoultsResourceid = new Array();
		jQuery("#noRecodeExamResultBody input[name='directlyImportExamResoults']:checked").each(function(){
			importExamresoultsResourceid.push(jQuery(this).val());
		});
		jQuery("#specialSubExamResultBody input[name='directlyImportExamResoults']:checked").each(function(){
			importExamresoultsResourceid.push(jQuery(this).val());
		});
		if(importExamresoultsResourceid.length>0){
			var $form = $("#examResultExamSubForm");
			...
			alertMsg.confirm("确认要保存并生成所选成绩吗？", {
                okCall: function(){
					$.ajax({
						type:'POST',
						url:"${baseUrl}/edu3/teaching/transcripts/upload-without-record-process.html",
						data:$form.serializeArray(),
						dataType:"json",
						cache: false,
						error: DWZ.ajaxError,
							...
							if(success ==false){
								...
								if(null!=errorList && errorList.length>0){
									for(var i=0;i<errorList.length;i++){
										if(""!=resultData[errorList[i]]){
											infoMsg +=	resultData[errorList[i]];
										}
									}
								}
								if(null!=saveList && saveList.length>0){
									for(var j=0;j<saveList.length;j++){	
										$("#specialSubExamResultBody  input[name='directlyImportExamResoults'][value='"+saveList[j]+"']").parent().parent().parent().html("");
										$("#noRecodeExamResultBody    input[name='directlyImportExamResoults'][value='"+saveList[j]+"']").parent().parent().parent().html("");
									}
								}	
								if(""!=infoMsg){
									alertMsg.warn(infoMsg);
								}
	...
	function revocationImprot(){
		if($("#revocationImprotDIV").css('display')=='block'){
			$("#revocationImprotDIV").hide();
		}else{
			$("#revocationImprotDIV").show();
		}
	}
	//选择清空类型
	function chooseRevocationType(revocationType){
		var url       = baseUrl+"/edu3/teaching/examinfo/findExamSubCourseList.html";
			...
			jQuery.ajax({
					...
					success:function(resultData){
						if(resultData != null && resultData.length != 0){
							jQuery("#examSubCourseSelect").children().remove();
							var insertHTML ="<option value=''>请选择课程</option>";
							//jQuery("#revocationImprotTypeSPN").hide();
							for(var i=0;i<resultData.length;i++){
								insertHTML += '<option value="'+resultData[i].resourceid+'">'+resultData[i].courseName+'</option>';
							}
							jQuery("#revocationImprotCourseSPN").show();
							jQuery("#examSubCourseSelect").html(insertHTML);
						}
					}
			});
			
		}else if(revocationType==0){
			
			jQuery("#revocationImprotCourseSPN").hide();
			alertMsg.confirm("确定要清空当前批次的所有课程成绩吗？",{
				okCall:function(){
					var revocationAllURL = baseUrl+"/edu3/teaching/revocationImprot-all.html";
					jQuery.ajax({
						type:'post',
						url:revocationAllURL,
						data:'examSubId='+examSubId,
						dataType:'json',
						success:function(resultData){
							...
							if(isSuccess==true){
								...
								if($("#exceptionExamResultBody")){
									$("#exceptionExamResultBody").remove();
								}
								alertMsg.warn(msg);
							}else{
								alertMsg.warn(msg);
	...
	//按课程清空成绩
	function revocationImprotByCourseProcess(){
		
		alertMsg.confirm("确定要清空当前批次所选课程的成绩吗？",{
			okCall:function(){
				var revocationByCourseURL = baseUrl+"/edu3/teaching/revocationImprot-course.html";
				var examSubId = jQuery("#examResultExamSubForm >input:id='examSubId'").val();
				var courseId  = jQuery("#examSubCourseSelect").val();
				jQuery.ajax({
						type:'post',
						url:revocationByCourseURL,
						data:'examSubId='+examSubId+"&courseId="+courseId,
						dataType:'json',
						success:function(resultData){
							var isSuccess = resultData['success'];
							var msg       = resultData['msg'];
							var courseId  = resultData['courseId'];
							if(isSuccess==true){
								jQuery("tr[id='"+courseId+"']").remove();
								//jQuery("#examResultsVoBody").children().remove();
								alertMsg.warn(msg);
							}else{
								alertMsg.warn(msg);
	...
	function checkExamResult(){
		...
		jQuery("#uploaderExamResultFileDIV > input:name='uploadExamResultFileId'").each(function(){
		...
		if(uploadExamResultFileId.length != 0){
			...
			jQuery.ajax({
				type:"post",
				url:url,
				data:"uploadExamResultFileId="+uploadExamResultFileId.toString()+"&examSubId="+examSubId,
				dataType:"json",
				success:function(resultData){
					var success = resultData['success'];
					var msg     = resultData['msg'];
					if(success ==false){
						...
						if(examResulstVolist.length>0){//当前考试批次中有相应的预约记录
							...
							for(var i=0;i<examResulstVolist.length;i++){	
								...
								/*var examAbnormitySelect = "<select name='examAbnormity"+examResults.examResultsResourceId+"'>";
								...
								jQuery("#examResultsVoBody").append("<tr ><td width='4%'><input type='checkbox' name='importExamresoultsResourceid' value="+examResults.examResultsResourceId+" autocomplete='off' /></td>"+
																	"<td width='4%'>"+examResults.sort+"</td>"+
																	"<td width='22%'>"+examResults.courseName+"</td>"+
																	"<td width='23%'>"+examResults.branchSchool+"</td>"+
																	"<td width='24%'>"+examResults.major+"</td>"+
																	"<td width='8%'>"+examResults.studyNo+"</td>"+
																	"<td width='8%'>"+examResults.name+"</td>"+
																	...
																	/*
																	"<td width='7%'><input type='text' name='usuallyScore"+examResults.examResultsResourceId+"'    align='middle' class='number' style='width:30px' value='"+examResults.usuallyScore+"'/></td>"+
																	"<td width='7%'><input type='text' name='integratedScore"+examResults.examResultsResourceId+"' align='middle' class='number' style='width:30px'  value='"+examResults.integratedScore+"'/></td>"+
																	"<td width='7%'>"+examAbnormitySelect+"</td></tr>"*/
						...
						if(hasNoRecode == true){//当前考试批次中没有相应的预约记录	
							if(noRecodeList.length>0){
								var noRecodeExamResultBody  = "<tbody id='noRecodeExamResultBody'><tr align='center'><td colspan='11'  style='color: red;text-align: center;font-weight: bolder' >以下成绩在所选的考试批次找不到记录或者已录入成绩！</td></tr>"
	                                ...
	                                //<th width='7%'>平时成绩</th> <th width='7%'>综合成绩</th><th width='7%'>成绩异常</th>
								for(var i=0;i<noRecodeList.length;i++){
									...
									examAbnormitySelect += "</select>";
									*/
									//noRecodeExamResultBody += "<input type='hidden' name='excelExamSubId' value='"+examResults.examSubId+"'/>";
	                              	//noRecodeExamResultBody += "<input type='hidden' name='excelCourseId' value='"+examResults.courseId+"'/>";
	                             	//noRecodeExamResultBody += "<input type='hidden' name='studyNo' value='"+examResults.studyNo+"'/>";
									...
									//noRecodeExamResultBody += "<td width='7%'><input type='text' name='usuallyScore"+examResults.studyNo+examResults.courseId+"' align='middle' class='number' style='width:30px' value='"+examResults.usuallyScore+"'/></td>";
									//noRecodeExamResultBody += "<td width='7%'><input type='text' name='integratedScore"+examResults.studyNo+examResults.courseId+"' align='middle' class='number' style='width:30px'  value='"+examResults.integratedScore+"'/></td>";
									//noRecodeExamResultBody += "<td width='7%'>"+examAbnormitySelect+"</td></tr>"
								}
							...
							if(specialSubList.length>0){
								...
								for(var i=0;i<specialSubList.length;i++){
									...
									examAbnormitySelect += "</select>"
	                              	...
	                              	//specialSubExamResultBody += "<input type='hidden' name='excelExamSubId' value='"+examResults.examSubId+"'/>";
	                              	//specialSubExamResultBody += "<input type='hidden' name='excelCourseId' value='"+examResults.courseId+"'/>";
	                              	//specialSubExamResultBody += "<input type='hidden' name='studyNo' value='"+examResults.studyNo+"'/>";
	...
	function initUploadExamResults(url){
		...
		$("#importExamResultsFile").uploadify({
	            ...
	            'auto'           : true, //自动上传
	            'scriptData'	 : {'formId':examSubId,'formType':'examResultsImportUncheck'},
	            'multi'          : false, //多文件上传
                ...
                onComplete: function (event, queueID, fileObj, response, data) { //上传成功回调函数
					...
					//每上传一个附件记录一个成绩日志
					addExamResultsLog(examSubId,response.split("|")[0]);
				},
   ...
   function deleteAttachFile(attid){
    ...
    //上传附件时增加日志
	function addExamResultsLog(examSubId,attid){
		jQuery.ajax({
					type:'post',
					 url:baseUrl+'/edu3/teaching/examResultsLog/add.html',
					data:'examSubId='+examSubId+"&optionType=examResultsImportUncheck&attid="+attid,
			    dataType:'josn',
