...
function confirmGraduateAudit(){
	alertMsg.confirm("确定将<font color='blue'>审核通过</font>的学生确认毕业么？<br>"+
...
function doGraduateAudit(){//确定毕业
	var graduateDate = $('#graduateDate_ga').val();
	if(""==graduateDate){
		alertMsg.warn("请您设置毕业日期。");
		return false;
	}
					...
					<table class="table" layouth="168" style="width: 100%;">
						...
						<tbody id="auditStusResults">
							<c:forEach items="${auditResults.result}" var="auditResult"
								varStatus="vs">
								<tr>
									<td>${auditResult.accountStatus}</td>
									<td>${auditResult.bloodType}</td>
									<td><c:if test="${auditResult.bornAddress=='1'}">
											<font color="blue">审核通过</font>
										</c:if> <c:if test="${auditResult.bornAddress=='0'}">
											<font color="red">审核不通过</font>
										</c:if></td>
									<td>${auditResult.bornDay}</td>
									<td>${auditResult.certNum}</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
