...
<title>学生退费明细记录</title>
<script type="text/javascript">
$(document).ready(function(){
    var total = parseInt("${total}");
	if(total<=0){
		alertMsg.warn("查询数据为空,(已有收据号的无法打印),无法打印"); 
		$.pdialog.close("RES_FINANCE_STUDENTPAYMENT_REFUND_COUNT");
	}
});
//根据查询条件打印缴费明细记录
function valReceiptNumber() {
	var receiptNumber_begin = $("#studentPaymentReturn_receiptNumber_begin");
	var receiptNumber_end = $("#studentPaymentReturn_receiptNumber_end");
	var drawer = $("#studentPaymentReturn_drawer");
	var total = parseInt("${total}");
	...
	$.ajax({
		type:"post",
		url:baseUrl+"/edu3/finance/studentPayment/studentPayment-refund-checkReceiptNumber.html",
...
<body>
	<div class="page">
		<div class="pageHeader">
			<form id="studentPaymentReturn_print_form" target=""
				...
				<div class="searchBar">
					<ul>
						<li><label>票据号开始：</label><input
							id="studentPaymentReturn_receiptNumber_begin" type="text"
					...
					<ul>
						<li><label>票据号结束：</label><input
							id="studentPaymentReturn_receiptNumber_end" type="text"
							name="receiptNumber_end" style="width: 22%;" /></li>
					</ul>
					<br />
					<ul>
						<li><label>开票人：</label><input
							id="studentPaymentReturn_drawer" type="text" name="drawer"
