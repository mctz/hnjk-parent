...
qq.FileUploader = function(o) {
	this._options = {
		...
		template : '<div class="qq-uploader">'
				+ '<div class="qq-upload-drop-area"><span>删除</span></div>'
				+ '<div class="qq-upload-button">上传</div>'
		...
		fileTemplate : '<li>' + '<span class="qq-upload-file"></span>'
				...
				+ '<span class="qq-upload-failed-text">失败</span>' + '</li>',
		...
		messages : {
			// serverError: "Some files were not uploaded, please contact
			// support and/or try again.",
			typeError : "{file} 为不允许上传文件. 仅仅 {extensions} 是允许上传.",
			sizeError : "{file} 太大, 超过了最大尺寸 {sizeLimit} 限制.",
...
qq.FileUploader.prototype = {
	 ...
	 * 
	 * First optional element is root for search, this._element is default
	 * value.
	 * 
	 * Usage 1. this._getElement('button'); 2. this._getElement(item, 'file');
	 */
	...
	_error : function(code, fileName) {
		...
		message = message.replace('{extensions}',
				this._options.allowedExtensions.join(', '));
		message = message.replace('{sizeLimit}', this
		...
		qq.attach(document, 'dragover', function(e) {
					if (isValidDrag(e)) {
						...
						if (dropArea == e.target
								|| qq.contains(dropArea, e.target)) {
							var effect = e.dataTransfer.effectAllowed;
							if (effect == 'move' || effect == 'linkMove') {
								e.dataTransfer.dropEffect = 'move'; // for FF
								// (only
								// move
								// allowed)
							} else {
								e.dataTransfer.dropEffect = 'copy'; // for
								// Chrome
							}
	...
	_validateFile : function(file) {
		...
		} else if (size && this._options.sizeLimit
	...
	_bindCancelEvent : function() {
		var self = this, list = this._getElement('list');
...
qq.UploadHandlerForm.prototype = {
	/**
	 * Adds file input to the queue Returns id to use with upload, cancel
	 */
	...
	upload : function(id, params) {
		...
		this._attachLoadEvent(iframe, function() {
					self._options.onComplete(id, fileName, self
