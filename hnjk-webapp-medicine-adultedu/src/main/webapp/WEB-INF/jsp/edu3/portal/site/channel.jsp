...
<title>${school}${schoolConnectName}-${channel.channelName }</title>
									...
									<c:forEach items="${channel.parent.children}" var="chan"
										...
										<tr>
											<td height="22" align="left" valign="bottom">
												<!-- 判断是否为外部链接 --> <c:choose>
													<c:when test="${chan.channelType!='outlink' }">
														<a
															href="${baseUrl }/portal/site/channel/show.html?id=${chan.resourceid }">
													</c:when>
													<c:otherwise>
														<a href="${chan.channelHref}" target="_blank">
													</c:otherwise>
												</c:choose> <!-- 突出当前栏目 --> <c:choose>
						...
						<td valign="top"><table width="816" height="30" border="0"
								...
								<tr>
									<td height="30"><span class="STYLE4">当前位置：<a
											href="#">${channel.channelName}</a></span></td>
								</tr>
							</table></td>
					</tr>
					<tr>
								...
								<c:forEach items="${articleList.result}" var="article"
									...
									<tr>
										<td align="left" width="80%"
											...
											<a
											href="${baseUrl}/portal/site/channel/show.html?id=${article.channel.resourceid}">【${article.channel.channelName}】</a>
											<a
											href="${baseUrl}/portal/site/article/show.html?id=${article.resourceid}">${article.title}</a>
										</td>
