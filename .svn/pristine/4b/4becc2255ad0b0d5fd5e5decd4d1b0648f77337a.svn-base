...
KE.setting = {
	...
	items : [
		...
		'flash', 'media', 'advtable', 'hr','math', 'emoticons', 'link', 'unlink', '|', 'about'
	],
	colorTable : [
	...
	this.show = function() {
		...
		if (arg.html !== undefined) {
			...
			if (arg.file === undefined) {
				iframe.src = KE.g[id].pluginsPath + arg.cmd + '.htm?' + param;
			} else {
	...
	click : function(id) {
		...
		var dialog = new KE.dialog({
			...
			check : function(id) {
				...
				var url = KE.$('url', dialogDoc).value;
				var mathContent = KE.$('mathContent', dialogDoc).value;
				var title = KE.$('imgTitle', dialogDoc).value;
				var width = KE.$('imgWidth', dialogDoc).value;
				var height = KE.$('imgHeight', dialogDoc).value;
				...
				if(mathContent == "" || mathContent.length <1){
					...
					if(mathContent.match(/<script[^>]*?>/) != null){
						alert("公式文本包含了脚本信息!");
						window.focus();
						KE.g[id].yesButton.focus();
						return false;
			...
			exec : function(id) {
				...
				var height = KE.$('imgHeight', dialogDoc).value;
				var border = KE.$('imgBorder', dialogDoc).value;
				if(mathContent.match(/<svg[^>]*?>/) != null){
					this.insert(id, mathContent, title, width, height, border);
				}else{
					this.insert(id, url+mathContent, title, width, height, border);
				}
...
KE.plugin['advtable'] = {
	...
	click : function(id, mode) {
		...
		this.dialog = new KE.dialog({
			id : id,
			cmd : cmd,
			file : 'advtable/advtable.htm?mode=' + mode,
			width : 420,
			height : 220,
			loadingMode : true,
