...
(function($) {
  ...
  $.extend($.flexselect.prototype, {
    ...
    renderControls: function() {
      ...
      }).addClass(this.select.attr("class")).val(selected.val().replace(/<[^>].*?>/g,""));
      ...
      }).addClass(this.select.attr("class")).val($.trim(selected.text().replace(/<[^>].*?>/g,"")));
    ...
    renderDropdown: function() {
      ...
      $.each(this.results, function() {
    	  var liobj = $("<li/>");
    	  if($.trim(this.name)==""){
    		  liobj.css({"height":"20px"});    		 
    	  }
    	 // alert(this.name);
    	  list.append(liobj.html(this.name));
    ...
    pickSelected: function() {
      ...
      if (selected) {
        this.input.val(selected.name.replace(/<[^>].*?>/g,""));
        this.hidden.val(selected.value.replace(/<[^>].*?>/g,""));
        this.picked = true;
