...
*/
package com.hnjk.extend.plugin.dbf;
...
public final class Utils {
	...
	/**
	 * 生成DBF
	 * @param dbfName dbfName path
	 * @param strutName field names
	 * @param strutType field types
	 * @param strutLength field length
	 * @param values data
	 */
	public static void createDbfFromArray(String dbfName,String[] strutName,byte[] strutType,int[] strutLength,Object[][] values){
		 //生成field
		...
		createDbfFromField(dbfName,fields,values);
	}
	 ...
	 * @param values
	 */
	public static void createDbfFromField(String dbfName,DBFField[] fields,Object[][] values){
		OutputStream fos = null;   
		try {
			 DBFWriter writer = new DBFWriter();   
			 writer.setFields(fields);   
			 for(int i=0;i<fields.length;i++) {  		  
			     writer.addRecord(values[i]);   
			   }   
