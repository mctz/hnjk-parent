				...
				<c:if test="${not empty studyStepList }">
					<div id="TabbedPanels1" class="TabbedPanels">
						...
						<div class="TabbedPanelsContentGroup">
							<c:forEach items="${studyStepList }" var="stu" varStatus="vs">
								<div class="TabbedPanelsContent">${fn:replace(stu.channelContent,newLine,'<br/>') }</div>
							</c:forEach>
						</div>
					</div>
				</c:if>
			...
			<div id="index_right">
				<p class="indent">
					<c:choose>
						...
						您好 <span class="t_blue t_strong">${user.cnName }</span>，欢迎学习本课程！本课程理论知识共 60 个。 
						</c:when>
						<c:otherwise>
						您好，欢迎学习本课程！本课程理论知识共 60 个。 由于您当前的身份为 <span class="t_blue t_strong">游客</span> ， 若想查看您的个人学习信息记录，请<a
				...
				<ul style="padding-left: 30px; line-height: 20px;">
					<c:forEach items="${studyCenterFilesList }" var="file"
						...
						<li style="list-style: decimal;"><c:choose>
								<%-- 外部附件 --%>
								<c:when test="${empty file.attach.resourceid }">
									<a href="${file.fileUrl }" title="${file.fileName }">${file.fileName }</a>
								</c:when>
								<%-- 本地附件 --%>
								<c:otherwise>
									<a href="javascript:void(0)"
				...
				<c:if test="${not empty studyCenterFilesList }">
					<p style="text-align: right">
						<a
							href="${baseUrl }/resource/course/coursefiles.html?type=1&courseId=${course.resourceid}">更多>>
						</a>
					</p>
				</c:if>
			</div>
