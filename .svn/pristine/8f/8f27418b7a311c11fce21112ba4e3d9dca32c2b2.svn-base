...
public class YearInfoServiceImpl extends BaseServiceImpl<YearInfo> implements IYearInfoService{
	...
	@Override
	public YearInfo findOrCreateYearInfoByFirstYear(Long firseYear) {
		YearInfo yearInfo = findUniqueByProperty("firstYear", firseYear);
		if (yearInfo == null) {
			yearInfo = new YearInfo();
			yearInfo.setFirstYear(firseYear);
			yearInfo.setYearName(firseYear+"学年度");
			yearInfo.setFirstTermWeekNum(20L);
			yearInfo.setSecondTermWeekNum(20L);
			yearInfo.setFirstMondayOffirstTerm(ExDateUtils.convertToDate(firseYear+"-03-01"));
			yearInfo.setFirstMondayOfSecondTerm(ExDateUtils.convertToDate(firseYear+"-09-01"));
			save(yearInfo);
		}
		return yearInfo;
	}
