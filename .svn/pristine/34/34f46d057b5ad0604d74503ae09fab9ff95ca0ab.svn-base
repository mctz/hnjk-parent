...
public class AbstractWorkflow implements Workflow {
    ...
    // 将AbstractWorkflow configuration 设置为静态,防止子类每次实例化时自动使用DefaultConfiguration
    ...
    public void setConfiguration(Configuration configuration) {
        AbstractWorkflow.configuration = configuration;
    }
    ...
    public void doAction(long id, int actionId, Map inputs) throws WorkflowException {
        ...
        List<HibernateCurrentStep> currentSteps = store.findCurrentSteps(id);
    ...
    public void doGetBack(long entryId) throws WorkflowException{
    	getPersistence().doGetBack(entryId);
    }
    
    @Override
    public void deleteEntry(long entryId) throws WorkflowException{
    	getPersistence().deleteEntry(entryId);
    }
    
    @Override
    public void updateState(HibernateStep step) throws WorkflowException{
    	getPersistence().updateStep(step);
    }
    ...
    @Override
    public long initialize(String workflowName, String userId, String unitId, int initialAction, Map inputs) throws InvalidRoleException, InvalidInputException, WorkflowException {
        WorkflowDescriptor wf = getConfiguration().getWorkflow(workflowName);

        WorkflowStore store = getPersistence();
        //构造流程实例
        HibernateWorkflowEntry workflowEntry = new HibernateWorkflowEntry();
        workflowEntry.setState(WorkflowEntry.CREATED);
        workflowEntry.setWorkflowName(workflowName);
        if(ExStringUtils.isNotEmpty(userId)) {
            workflowEntry.setUserId(userId);
        }
        if(ExStringUtils.isNotEmpty(unitId)) {
            workflowEntry.setUnitId(unitId);
        }
        
        WorkflowEntry entry = store.createEntry(workflowEntry);
    ...
    public boolean canInitialize(String workflowName, int initialAction, Map inputs) {
        ...
        WorkflowEntry mockEntry = new WorkflowEntry() {
			private static final long serialVersionUID = -6192413103149540076L;
    ...
    public WorkflowEntry getEntry(long id) throws WorkflowException {		
		return getPersistence().findEntry(id);
	}    

	@Override
	public Page queryCurrentWorksByPage(Page objPage, String userId,	Map<String, Object> paramsMap) throws WorkflowException {
		
		return getPersistence().queryCurrentWorksByPage(objPage, userId, paramsMap);
	}

	
	@Override
	public Page queryHistoryWorksByPage(Page objPage, String userId,	Map<String, Object> paramsMap) throws WorkflowException {
		return getPersistence().queryHistoryWorksByPage(objPage, userId, paramsMap);
	}
	
	
	@Override
	public Map<String, Object> getWorkflowDescriptMeta(String workflowname,long wf_id, String type)	throws WorkflowException {
		WorkflowDescriptor descript = getWorkflowDescriptor(workflowname);
		if(null != descript){
			List<HibernateCurrentStep> steps = getCurrentSteps(wf_id) ;
			if(null != steps && !steps.isEmpty()){
				HibernateCurrentStep step = steps.get(0);				
				if("step".equals(type)){
					StepDescriptor stepDescript = descript.getStep(new Long(step.getStepId()).intValue());
					return stepDescript.getMetaAttributes();
	...
	private List getAvailableActionsForStep(WorkflowDescriptor wf, Step step, Map transientVars, PropertySet ps) throws WorkflowException {
        List l = new ArrayList();
        StepDescriptor s = wf.getStep(step.getStepId());
     ...
     */
    private List getAvailableAutoActionsForStep(WorkflowDescriptor wf, Step step, Map transientVars, PropertySet ps) throws WorkflowException {
        List l = new ArrayList();
        StepDescriptor s = wf.getStep(step.getStepId());
    ...
    protected boolean passesCondition(ConditionDescriptor conditionDesc, Map transientVars, PropertySet ps, int currentStepId) throws WorkflowException {
        ...
        if (currentStepId != -1) {
            ...
            if ((stepId != null) && "-1".equals(stepId)) {
    ...
    protected boolean transitionWorkflow(WorkflowEntry entry, List currentSteps, WorkflowStore store, WorkflowDescriptor wf, ActionDescriptor action, Map transientVars, Map inputs, PropertySet ps) throws WorkflowException {
        ...
        // 设置附带属性 -将要变成历史的当前记录
        if(inputs != null) {
        	HibernateCurrentStep cs = ((HibernateCurrentStep)step);
        	cs.setOpinion(String.valueOf(inputs.get("opinion")));
        	// 如果存在手动设置的owner 要传递
        	if(inputs.containsKey("owner")) {
        		transientVars.put("owner", inputs.get("owner"));
        	}       	
        ...
        if (theResults[0].getSplit() != 0) {
            ...
            List<HibernateHistoryStep> historySteps = store.findHistorySteps(entry.getId());

            for (HibernateHistoryStep historyStep : historySteps) {
    ...
    private Step createNewCurrentStep(ResultDescriptor theResult, WorkflowEntry entry, WorkflowStore store, int actionId, Step currentStep, long[] previousIds, Map transientVars, PropertySet ps) throws WorkflowException {
        try {
            ...
            // 此处修改ower
            owner = transientVars.containsKey("owner") ? transientVars.get("owner").toString() : owner;
