...
public class WorkflowDescriptor extends AbstractDescriptor implements Validatable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3365016811859462974L;
    ...
    protected ConditionsDescriptor globalConditions = null;//条件
    protected List commonActionsList = new ArrayList(); // for preserving order
    protected List<ActionDescriptor> globalActions = new ArrayList<ActionDescriptor>();//全局动作
    protected List<ActionDescriptor> initialActions = new ArrayList<ActionDescriptor>();//初始化的动作
    protected List<JoinDescriptor> joins = new ArrayList<JoinDescriptor>();//连接
    protected List<RegisterDescriptor> registers = new ArrayList<RegisterDescriptor>();//注册人
    protected List<SplitDescriptor> splits = new ArrayList<SplitDescriptor>();//分割
    protected List<StepDescriptor> steps = new ArrayList<StepDescriptor>();//步骤
    protected Map commonActions = new HashMap();//公共动作
    protected Map<String,Object> metaAttributes = new HashMap<String,Object>();//扩展属性
    protected Map timerFunctions = new HashMap();//调度器函数
    protected String workflowName = null;//工作流名称
    ...
    */
    ...
    public JoinDescriptor getJoin(int id) {
    	for(JoinDescriptor joinDescriptor : joins){
    ...
    public boolean removeAction(ActionDescriptor actionToRemove) {
        // global actions
    	for(ActionDescriptor actionDescriptor  : getGlobalActions()){
     ...
     * 校验
     * (non-Javadoc)
     * @see com.opensymphony.workflow.util.Validatable#validate()
     */
    @Override
	...
	public void writeXML(PrintWriter out, int indent) {
        ...
        if (timerFunctions.size() > 0) {//写入timer函数
            XMLUtil.printIndent(out, indent++);
        ...
        if (getGlobalConditions() != null) {//写入全局条件
            XMLUtil.printIndent(out, indent++);
        ...
        XMLUtil.printIndent(out, indent++);//空格
        
        out.println("<initial-actions>");//写入初始化动作
        ...
        if (globalActions.size() > 0) {//写入全局动作
            XMLUtil.printIndent(out, indent++);
        ...
        if (commonActions.size() > 0) {//写入公共动作
            XMLUtil.printIndent(out, indent++);
    ...
    protected void init(Element root) {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if ("meta".equals(child.getNodeName())) {//获取mete信息
        ...
        if (globalConditionsElement != null) {//获取全局条件
        ...
        for (int i = 0; i < initialActions.size(); i++) {//获取初始化动作
        ...
        if (globalActionsElement != null) {//获取全局动作
        ...
        if (commonActionsElement != null) {//获取共有动作
        ...
        if (timerFunctionsElement != null) {//获取timer函数
        ...
        for (int i = 0; i < steps.size(); i++) {//获取步骤
