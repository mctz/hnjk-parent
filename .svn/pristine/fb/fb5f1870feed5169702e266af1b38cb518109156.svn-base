...
//			Map<String, String> userNameMap = (Map<String, String>)application.getAttribute("userNameMap");
            ...
            if("multi".equals(loginType)){//多点登录
            	userKey += "_"+httpSession.getId();
            } else {// 单点登录
            	  // 将之前的session销毁
                if(userSessoinMap.containsKey(userKey)){
            ...
            UserOperationLogsHelper.saveUserOperationLogs(HttpHeaderUtils.getIpAddr(servletRequest), "8", UserOperationLogs.LOGIN, "账号登录，当前登录模式(是否允许多账号登录)："+loginType+"||浏览器名称："+userAgent.getBrowser().getName()+"||版本:"+userAgent.getBrowserVersion().getVersion()+"||操作系统版本:"+userAgent.getOperatingSystem().getName()+"||userAgent:"+servletRequest.getHeader("user-agent"));
            if(!userSessoinMap.containsKey(userKey)){
            	userSessoinMap.put(userKey, httpSession);
