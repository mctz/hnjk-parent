...
public class UniversalExamDetailsVO {
	...
	private String unScore;//免考：1
	private String abnormalType;//缓考：1
	...
	public String getUsuallyScore() {
		String result="";		
		if(ExStringUtils.isNotBlank(this.abnormalType)){
			if(this.abnormalType.equals("1")){
				result="缓考";
			}				
		}else if(ExStringUtils.isNotBlank(this.unScore)){
			if(this.unScore.equals("1")){
				result="免考";
			}
		}else{
			result=this.studentId!=null?ScoreEncryptionDecryptionUtil.getInstance().decrypt(this.studentId,this.usuallyScore):this.usuallyScore;
		}		
		return result;
	}
	...
	public String getWrittenScore() {
		...
		if(ExStringUtils.isNotBlank(this.abnormalType)){
			...
			result=this.studentId!=null?ScoreEncryptionDecryptionUtil.getInstance().decrypt(this.studentId,this.writtenScore):this.writtenScore;
		}
		return result;
	}
