...
/**
 * This is the property set implementation for storing properties using
 * Hibernate.
 * <p>
 * 
 * <b>Required Args</b>
 ...
 * </ul>
 * 
 * if "<b>sessionFactory</b> - hibernate sessionFactory" is not passed as an arg
 * then init will use: <br />
 * <b>hibernate.*</b> - config params needed to create a hibernate
 * sessionFactory in the propertyset config xml. <br />
...
public class HibernatePropertySet extends AbstractPropertySet {
	...
	public void init(Map config, Map args) {
		...
		this.configProvider = (HibernateConfigurationProvider) args.get("configurationProvider"); //$NON-NLS-1$
		...
		if (this.configProvider == null) {
			...
			if (configProviderClass != null) {
				...
				try {
					this.configProvider = (HibernateConfigurationProvider) ClassLoaderUtil.loadClass(configProviderClass, this.getClass()).newInstance();
				} catch (Exception e) {
				...
				if (log.isDebugEnabled()) {
					log.debug("Setting up property set with DefaultHibernateConfigurationProvider");//$NON-NLS-1$
				}
			...
			if (log.isDebugEnabled()) {
				log.debug("Setting up property set with hibernate provider passed in args."); //$NON-NLS-1$
			}
		}
	}
	...
	protected void setImpl(int type, String key, Object value) throws PropertyException {
		...
		switch (type) {
			...
			case DATE:
				...
				break;
			default:
				throw new PropertyException("type " + type + " not supported"); //$NON-NLS-1$ //$NON-NLS-2$
		}
		item.setType(type);
